...
import { GeneralModel, WorkerModel, ClientModel } from 'modules/models';
import { useForm } from 'utils/useForm';
import { IFormRules, isEmpty } from 'utils';
...
const StepEditor = ({
  ...
  const { model, formRules, errors, hasChanges, onChange, update, discardChanges, updateRules, onSubmit, resetErrors, onValidate } = useForm<
    ClientModel.IClient
  >({
  ...
  useEffect(() => {
    if (open) {
      const rules = getRules(model)[step];
      Object.keys(rules).forEach(key => {
        onValidate(key);
      });
    }
  }, [formRules, open, getRules, onValidate, step]); // eslint-disable-line
  ...
  const onDiscard = useCallback(() => {
    ...
    resetErrors();
    clearErrors();
  }, [hasChanges, currentEntity, discardChanges, resetErrors, clearErrors]);
