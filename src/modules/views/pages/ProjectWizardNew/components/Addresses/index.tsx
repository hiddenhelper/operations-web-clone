...
import { ProjectModel, AddressModel } from 'modules/models';
...
const Addresses = ({ onChange, model, errors }: IAddressesProps) => {
  ...
  const labelRules = FormRules.projectNew.AddressesLabelRules;
  ...
  return (
    <>
      <Card title="Job Site Address">
        <AddressForm
          onChange={onChange}
          addressModel={jobSiteAddress}
          errors={errors}
          modelProperty={ProjectModel.ProjectFields.JOB_SITE_ADDRESS}
          labelRules={labelRules}
        />
      </Card>
      <Locations errors={errors} status={model.status} locationsList={model.locations} onChange={onChange} labelRules={labelRules} />
      <Card title="Badging Site Address" styleClass={classes.addressWrapper}>
        ...
        {!model.badgingSiteAddressMatchesJobSiteAddress && (
          <AddressForm
            addressModel={model.badgingSiteAddress}
            onChange={onChange}
            modelProperty={ProjectModel.ProjectFields.BADGING_ADDRESS}
            errors={errors}
            labelRules={labelRules}
          />
        )}
      ...
      <Card title="Mailing Address" styleClass={classes.addressWrapper}>
        ...
        {model.mailingAddressMatchingType === ProjectModel.MailingAddressMatchingType.NONE && (
          <AddressForm
            addressModel={model.mailingAddress}
            onChange={onChange}
            modelProperty={ProjectModel.ProjectFields.MAILING_ADDRESS}
            errors={errors}
            labelRules={labelRules}
          />
        )}
