...
const Locations = ({ onChange, locationsList, status, errors, labelRules }: ILocationsProps) => {
        ...
        return (
          <div key={index}>
            <Grid data-testid="user-row-item" container={true}>
              <Grid item={true} xs={showDelete ? 11 : 12}>
                <Grid container={true}>
                  <Grid container={true} className={`${formClasses.rowsWrapper} ${locationStyles.field}`}>
                    <div className={locationStyles.errorInputWrapper}>
                      <ControlledError
                        show={errors.locations === 'is required' || !!getErrors('locations', index)}
                        error={
                          errors.locations === 'is required'
                      ...
                      >
                        <ControlledInput label="Location Name" required={labelRules?.locations?.required} showMark={labelRules?.locations?.required}>
                          <TextField
                            autoComplete="off"
                            variant="outlined"
