...
import { GeneralModel, ConsentFormModel, ProjectModel, ResourceModel, UserModel, PaymentModel } from '../../../../models';
...
const Review = ({
  ...
  const handleEditGeneralInformation = useCallback(() => {
    editAction(ProjectModel.ProjectStep.GENERAL_INFORMATION);
  }, [editAction]);
  ...
  const consentFormLegalsBody = useCallback(
    list =>
      list.map(item => (
        <Grid key={item.id} container={true} spacing={0} className="cardBody-row">
      ...
      <Card
        ...
        secondaryAction={
          edit ? (
            ...
            <ReviewButton
              stepKey={ProjectModel.ProjectStep.GENERAL_INFORMATION}
              completedFields={completedFields[ProjectModel.ProjectStep.GENERAL_INFORMATION]}
          ...
          <Grid container={true} spacing={0} className={'cardBody-row'}>
            <Grid item={true} xs={6} className={'cardBody-item'}>
              ...
              <Grid container={true}>
                ...
                <Grid item={true} xs={6} className={'cardBody-item'}>
                  <span className={`${cardGlobalClasses.cardFontAccent} ${cardGlobalClasses.noWrap}`}>{getDefaultValue(model.timeZone?.name)}</span>
                </Grid>
              </Grid>
            </Grid>
          </Grid>
              ...
              {model.mustComplyWithNycLL196 && (
                <>
                  ...
                  <Grid container={true}>
                    ...
                    <Grid item={true} xs={10} className={'cardBody-item'}>
                      <span className={`${cardGlobalClasses.cardFontAccent} ${cardGlobalClasses.noWrap}`}>{getDefaultValue(model.permitHolder)}</span>
                    </Grid>
                    <Grid item={true} xs={2} className={'cardBody-item'}>
                      Permit Number:
                    </Grid>
                    <Grid item={true} xs={10} className={'cardBody-item'}>
                      <span className={`${cardGlobalClasses.cardFontAccent} ${cardGlobalClasses.noWrap}`}>{getDefaultValue(model.permitNumber)}</span>
                    </Grid>
                    <Grid item={true} xs={2} className={'cardBody-item'}>
                      Licence Number:
                    </Grid>
                    <Grid item={true} xs={10} className={'cardBody-item'}>
                      <span className={`${cardGlobalClasses.cardFontAccent} ${cardGlobalClasses.noWrap}`}>{getDefaultValue(model.licenseNumber)}</span>
                    </Grid>
                  </Grid>
                </>
              )}
